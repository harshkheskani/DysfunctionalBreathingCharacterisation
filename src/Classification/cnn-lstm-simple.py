...
class OSA_CNN_LSTM_Simple(nn.Module):
    """
    Simple CNN-LSTM model for sleep apnea detection.
    Similar to your proposed model but with improved initialization and structure.
    """
    def __init__(self, n_features, n_outputs, n_timesteps):
        super(OSA_CNN_LSTM_Simple, self).__init__()
        ...
        # Reduced pooling to preserve temporal information
    ...
    def forward(self, x):
        ...
        # For bidirectional LSTM, we concatenate forward and backward final states
...
def main():
    ...
    model = OSA_CNN_LSTM_Simple(n_features=n_features, n_outputs=N_OUTPUTS, n_timesteps=n_timesteps).to(device)
    print("\nPyTorch OSA_CNN_LSTM_Simple model created and moved to device.")
