...
class OSA_CNN_LSTM_Enhanced(nn.Module):
    """
    Enhanced CNN-LSTM model with residual connections and multi-scale convolutions.
    Combines the best of both your attention model and CNN-LSTM approach.
    """
    def __init__(self, n_features, n_outputs, n_timesteps):
        super(OSA_CNN_LSTM_Enhanced, self).__init__()
...
def main():
    ...
    model = OSA_CNN_LSTM_Enhanced(n_features=n_features, n_outputs=N_OUTPUTS, n_timesteps=n_timesteps).to(device)
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
        ...
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.2, patience=5, min_lr=1e-6)
        criterion = nn.CrossEntropyLoss()
        early_stopping = EarlyStopping(patience=20, verbose=False, path=f'lono_checkpoint_fold_lstm_enhanced_{fold}.pt')
